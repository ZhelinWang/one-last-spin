	...
	for act in actions:
		...
		if op == "prevent_decrease" and target_token == source_token:
			...
			if kind == "mult" and float(step.get("factor", 1.0)) < 1.0:
				return null
		elif op == "lock_value" and target_token == source_token:
			# Cancel any step targeting self
			return null
	return step
...
func _once_key(source_token: Resource) -> String:
	...
	return "__once_effect:" + str(id) + tok_part
...
func _roll_amount(act: AbilityAction, ctx: Dictionary, source_token: Resource = null) -> int:
	...
	if hi > lo:
		# Shared roll support
		var use_shared := String(act.shared_roll_key).strip_edges() != ""
		if use_shared and ctx is Dictionary:
			var map_any = ctx.get("__shared_rolls", {})
			var map: Dictionary = map_any if map_any is Dictionary else {}
			var tok_id: int = 0
			if source_token != null and (source_token as Object).has_method("get_instance_id"):
				tok_id = int((source_token as Object).get_instance_id())
			var key := String(act.shared_roll_key) + "|" + str(id) + "|" + str(tok_id)
			if map.has(key):
				return int(map.get(key))
			var rng0: RandomNumberGenerator = _mk_rng(ctx)
			var roll: int = int(rng0.randi_range(lo, hi))
			map[key] = roll
			ctx["__shared_rolls"] = map
			return roll
		var rng: RandomNumberGenerator = _mk_rng(ctx)
...
func _choose_offsets(candidates: Array[int], act: AbilityAction, ctx: Dictionary) -> Array[int]:
	var mode: String = String(act.choose_mode)
	var cnt: int = max(1, int(act.choose_count))
	...
	if mode == "N Random":
		var rng2: RandomNumberGenerator = _mk_rng(ctx)
		var pool: Array = candidates.duplicate()
		var out: Array[int] = []
		var want: int = min(cnt, pool.size())
		for i in range(want):
			var idx: int = rng2.randi_range(0, pool.size() - 1)
...
func build_steps(ctx: Dictionary, contrib: Dictionary, source_token: Resource) -> Array:
	...
	for act in actions:
		...
		if not _conditions_pass(ctx, contrib, [], source_token, act.conditions):
			continue
		if op == "add":
			var amt: int = _roll_amount(act, ctx, source_token)
			if amt != 0:
	...
	for act in actions:
		...
		match op:
			"permanent_add":
				...
				var amt_pa := int(_roll_amount(act, ctx, source_token))
				if amt_pa == 0:
					pass
				elif tkp == "choose":
					var d_pa := {
			...
			"destroy":
				...
				if tkd == "tag":
					...
					var count: int = max(1, int(act.choose_count))
			...
			"replace_from_choices":
				...
				if choices.size() == 0:
					continue
				if tkc == "choose":
					var cntc: int = max(1, int(act.choose_count))
					for i in range(cntc):
			...
			"spawn_random_by_tag":
				var cnt_tag: int = max(1, int(act.choose_count))
			...
			"guarantee_next_loot_rarity":
				...
				if rar.strip_edges() != "":
					var count: int = max(1, int(_roll_amount(act, ctx)))
...
# ---------- on removed (inventory/board mutations when this token is removed) ----------
func build_on_removed_commands(ctx: Dictionary, removed_token: Resource, _source_token: Resource) -> Array:
	if trigger != TokenAbility.Trigger.ON_REMOVED:
		return []
	if actions.is_empty():
		return []
	var out: Array = []
	...
	for act in actions:
		if act == null: continue
		var op: String = String(act.op).strip_edges().to_lower()
		match op:
			"adjust_run_total":
				var amt := int(act.amount)
				if amt != 0:
			...
			"destroy_inventory_tag":
				var ttag := String(act.inventory_tag)
				var maxd := int(act.max_destroy)
				out.append({"op": "destroy_inventory_tag", "target_tag": ttag, "max_destroy": maxd})
			"destroy_inventory_name":
				var iname := String(act.inventory_name)
				var maxd2 := int(act.max_destroy)
				out.append({"op": "destroy_inventory_name", "target_name": iname, "max_destroy": maxd2})
			"replace_inventory_tag":
				var rtag := String(act.inventory_tag)
				var rpath := String(act.token_path)
				var mr := int(act.max_replace)
				out.append({"op": "replace_inventory_tag", "target_tag": rtag, "token_path": rpath, "max_replace": mr, "preserve_tags": bool(act.preserve_tags_for_replace)})
			"replace_inventory_name":
				var rname := String(act.inventory_name)
				var rpath2 := String(act.token_path)
				var mr2 := int(act.max_replace)
				out.append({"op": "replace_inventory_name", "target_name": rname, "token_path": rpath2, "max_replace": mr2, "preserve_tags": bool(act.preserve_tags_for_replace)})
			"promote_killer_to_path":
				var p := String(act.token_path).strip_edges()
				if p != "":
					var killer = ctx.get("destroyed_by_token") if ctx is Dictionary and ctx.has("destroyed_by_token") else null
			...
			"spawn_random_by_tag":
				...
				out.append({"op": "spawn_random_by_tag", "target_tag": String(target_tag), "count": cnt_tag, "source": src})
			"spawn_random_by_rarity":
				var cnt_r: int = max(1, int(act.choose_count))
				out.append({"op": "spawn_random_by_rarity", "rarity": String(act.rarity), "count": cnt_r, "source": src})
			"replace":
				var path := String(act.token_path)
				if path.strip_edges() != "":
					out.append({"op": "replace_at_offset", "offset": 0, "token_path": path, "set_value": -1, "preserve_tags": false, "source": src})
			"permanent_add":
				var amt2 := int(_roll_amount(act, ctx, removed_token))
				if amt2 != 0:
					out.append({"op": "permanent_add", "target_kind": "self", "amount": amt2, "destroy_if_zero": false, "source": src})
			"adjust_run_total_by_self_fraction":
				var mul := float(act.factor)
				if mul <= 0.0:
					mul = 1.0
				var v := 0
				if removed_token != null and (removed_token as Object).has_method("get"):
					var raw = removed_token.get("value")
					if raw != null:
						v = int(raw)
				var payout := int(round(max(0.0, float(v)) * mul))
				if payout != 0:
					out.append({"op": "adjust_run_total", "amount": payout, "source": src})
			_:
				pass
	return out

# ---------- on acquire (when token is added to inventory) ----------
func on_added_to_inventory(board_tokens: Array, ctx: Dictionary, source_token: Resource) -> void:
	if trigger != TokenAbility.Trigger.ON_ACQUIRE:
		return
	if actions.is_empty():
		return
	var sr: Variant = ctx.get("spin_root") if ctx is Dictionary and ctx.has("spin_root") else null
	var mgr: Variant = ctx.get("coin_mgr") if ctx is Dictionary and ctx.has("coin_mgr") else null
	var rng: RandomNumberGenerator = _mk_rng(ctx)
	for act in actions:
		if act == null: continue
		var op: String = String(act.op).strip_edges().to_lower()
		match op:
			"spawn_token_in_inventory":
				...
				if p == "":
					continue
				var cnt: int = max(1, int(act.choose_count))
				if sr != null and (sr as Object).has_method("_insert_token_replacing_empties"):
					var res: Resource = ResourceLoader.load(p)
					if res is Resource:
						(sr as Object).call("_insert_token_replacing_empties", (res as Resource), int(cnt))
				else:
					# Fallback: append duplicates directly
					for i in range(cnt):
						var res2: Resource = ResourceLoader.load(p)
						if res2 is Resource:
							var dup: Resource = (res2 as Resource).duplicate(true)
							if board_tokens is Array: (board_tokens as Array).append(dup)
			"spawn_random_by_tag":
				var tag: String = String(act.target_tag)
				var cnt_tag: int = max(1, int(act.choose_count))
				if sr != null and (sr as Object).has_method("_insert_token_replacing_empties") and mgr != null and (mgr as Object).has_method("loot_scan_root"):
					# Fall back to scan via manager helper
					var root: String = String((mgr as Object).get("loot_scan_root")) if (mgr as Object).has_method("get") else "res://tokens"
					...
					if da != null:
						...
						for f in files:
							if f.ends_with(".tres"):
								var p2 = root.path_join(f)
								var resx: Resource = ResourceLoader.load(p2)
								if resx != null and (resx as Object).has_method("get") and _token_has_tag(resx, tag):
									paths.append(p2)
					for i in range(cnt_tag):
						if paths.is_empty(): break
						var pick := paths[rng.randi_range(0, paths.size()-1)]
						var res_pick = ResourceLoader.load(pick)
						if res_pick is Resource:
							(sr as Object).call("_insert_token_replacing_empties", (res_pick as Resource), 1)
			"spawn_random_by_rarity":
				var cnt_r = max(1, int(act.choose_count))
				var root2 := "res://tokens"
				if mgr != null and (mgr as Object).has_method("get"):
					var lp = (mgr as Object).get("loot_scan_root")
					if typeof(lp) == TYPE_STRING: root2 = String(lp)
				for i in range(cnt_r):
					# brute scan and pick
					var da2 = DirAccess.open(root2)
					var paths2: Array[String] = []
					if da2 != null:
						var files2 = da2.get_files()
						for f2 in files2:
							if f2.ends_with(".tres"):
								var p3 = root2.path_join(f2)
								var res3 = ResourceLoader.load(p3)
								if res3 != null and (res3 as Object).has_method("get") and String(res3.get("rarity")).to_lower() == String(act.rarity).to_lower():
									paths2.append(p3)
					if paths2.is_empty():
						break
					var pick2 := paths2[rng.randi_range(0, paths2.size()-1)]
					var res_pick2 = ResourceLoader.load(pick2)
					if res_pick2 is Resource and sr != null:
						(sr as Object).call("_insert_token_replacing_empties", (res_pick2 as Resource), 1)
			"replace_self_with_random_inventory":
				# On acquire: transform this token into a random inventory token
				var candidates: Array = []
				if board_tokens is Array:
					for t in board_tokens:
						if t == null or not (t as Object).has_method("get"): continue
						if bool(act.exclude_self) and t == source_token: continue
						if bool(act.require_different_name):
							var n1 := String(source_token.get("name")) if source_token.has_method("get") else ""
							var n2 := String(t.get("name"))
							if n1 == n2: continue
						candidates.append(t)
				if candidates.size() > 0 and sr != null:
					var pick_any = candidates[rng.randi_range(0, candidates.size()-1)]
					if (sr as Object).has_method("replace_token_in_inventory"):
						(sr as Object).call_deferred("replace_token_in_inventory", source_token, pick_any)
	# Update UI if spin_root present
	if sr != null and (sr as Object).has_method("_update_inventory_strip"):
		(sr as Object).call_deferred("_update_inventory_strip")
	if sr != null and (sr as Object).has_method("_refresh_inventory_baseline"):
		(sr as Object).call_deferred("_refresh_inventory_baseline")
		...
		match op:
			...
			"permanent_add_from_last_destroyed":
				...
				if d_tok != null and (d_tok as Object).has_method("get"):
					...
					if vv != null:
						v_d = int(vv)
				var numer_ld: int = max(0, int(act.gain_numer))
				var denom_ld: int = max(1, int(act.gain_denom))
					...
					if delta_m != 0:
						...
						if contribs is Array and not contribs.is_empty():
							...
							if not cands.is_empty():
								...
								steps2.append({"source":"ability:%s" % str(id), "kind":"mirror", "desc":"mirror change", "delta": delta_m})
								pick["steps"] = steps2
			"lock_value":
				if target_token == source_token and int(new_val) != int(prev_val):
					var lock_key := "__lock_value_guard"
					if ctx is Dictionary and bool(ctx.get(lock_key, false)):
						ctx[lock_key] = false
						return
					if ctx is Dictionary:
						ctx[lock_key] = true
					if source_token != null and (source_token as Object).has_method("set"):
						(source_token as Object).set("value", int(prev_val))
					if target_contrib is Dictionary:
						target_contrib["base"] = int(prev_val)
						target_contrib["delta"] = 0
						target_contrib["mult"] = 1.0
						if ctx is Dictionary:
							var fv: Variant = ctx.get("__force_value_sync_offsets", null)
							var offs: Array = []
							if fv is Array:
								offs = (fv as Array).duplicate()
							var offsv := int(target_contrib.get("offset", 0))
							if not offs.has(offsv):
								offs.append(offsv)
							ctx["__force_value_sync_offsets"] = offs
			_:
				pass
